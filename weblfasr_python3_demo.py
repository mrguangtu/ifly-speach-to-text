...
appid = "77b9422a"
secret_key = "c9a30ab16d369c9ec0b1d198a0a54a16"
...
class ResponseParse(object):
    ...
    def json2txt(self):
        jcontent = {}
        # j = open(self.json_file_path,'r',encoding='utf-8')
        # string = j.read()
        with open(self.json_file_path,'r',encoding='utf-8') as j:
            # tmp = demjson.decode(j.read())
            jcontent = json.load(j)
        sentences = eval(jcontent['data'])

        filename = time.strftime("%Y-%m-%d--%H-%M-%S.md")
        ...
        with open(filename,'w',encoding='utf-8') as f:
            ...
            for s in sentences:
                if s['speaker'] == former:
                    ...
                    start=float(s["bg"])/1000
                    hour=round(start//3600)
                    mininute=round(start%3600//60)
                    second=round(start%60)
                    timestr=f"{hour:>5d}:{mininute:>02d}:{second:<8d}"      
                    f.write(f'\n\n`{timestr}`\t**speaker {s["speaker"]}:**\n{s["onebest"]}\n')
                    former = s['speaker']
        return filename
...
if __name__ == '__main__':
    ...
    if os.path.exists(target_file):
        api = RequestApi(appid=appid, secret_key=secret_key, upload_file_path=target_file)
        json_file = api.all_api_request()
        print(f"{ResponseParse(json_file).json2txt()} 转写完成。")
        # print(target_file)
        # print(sys.argv)
        time.sleep(5)
    else:
